...
def gradientTest1(x, grad, f, theta, bias, y_mat):
    ...
    cost, grad_theta, grad_b, _ = softmaxRegression(theta_L=theta, x_L=x, b_L=bias, y_mat=y_mat)

    for eps in epsilonArray:
        theta_diff = theta.copy()
        theta_diff = theta_diff + (d * eps).reshape(2, 1)
        epsilon_cost, grad_theta, grad_b, _ = softmaxRegression(theta_L=theta_diff, x_L=x, b_L=bias, y_mat=y_mat)
